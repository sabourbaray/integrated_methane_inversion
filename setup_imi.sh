...
if "$SetupTemplateRundir"; then
    ...
    if "$UseEmisSF"; then
	OLD="use_emis_scale_factor: false"
	NEW="use_emis_scale_factor: true "
	sed -i "s/$OLD/$NEW/g" geoschem_config.yml
    fi
    if "$UseOHSF"; then
    ...
    if [ ! -z "$NestedRegion" ]; then
	sed -i -e "s:GEOS_0.25x0.3125\/GEOS_FP:GEOS_${gridResLong}_${NestedRegion}\/${metDir}:g" HEMCO_Config.rc
	sed -i -e "s:GEOS_0.25x0.3125\/GEOS_FP:GEOS_${gridResLong}_${NestedRegion}\/${metDir}:g" HEMCO_Config.rc.gmao_metfields
...
if  "$DoPreview"; then
   #set -x
    ...
    if "$UseSlurm"; then
        ...
        qsub -W block=true ./${RunName}_Preview.run; wait;
    fi
	...
	export PYTHONPATH=${PYTHONPATH}:${InversionPath}/src/inversion_scripts/
	chmod +x $preview_file 
        qsub -l ncpus=32,mem=128gb -W block=true -- $preview_file $InversionPath $config_path $state_vector_path $preview_dir $tropomi_cache; wait;
    fi
...
if "$SetupJacobianRuns"; then
	...
	sed -i -e "/export OMP_NUM_THREADS=32/d" \
	       -e "/cd/d" ${name}.run
	rm -f ch4_run.template
	chmod 755 ${name}.run
